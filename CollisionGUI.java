 ...
 * @author Mason Childers, 23W, filling in code for collision
 */
public class CollisionGUI extends DrawingGUI {
	...
	public void draw(Graphics g) {
		...
		// Ask the colliders to draw themselves in red. ISSUE COULD BE HERE
	...
	private void findColliders() {
		...
		// for each blob in the list of blobs...
		for (Blob blob : blobs) {
			// if this is the first blob added, initialize the quadtree
			if (blobTree == null) blobTree = new PointQuadtree<>(blob, 0, 0, 800, 600);
			// else, insert this new blob
			else blobTree.insert(blob);
		}
		// For each blob, see if anybody else collided with it. Starting by initializing the list of colliders
		colliders = new ArrayList<>();
		// for each blob in the list of blobs
		for (Blob blob : blobs) {
			// create a list of all points intersecting with that blob
			List<Blob> blobColliders = blobTree.findInCircle(blob.x, blob.y, 2*blob.r);
			// remove that blob since the list contains itself
			blobColliders.remove(blob);
			// add each of those blobs to the list of colliders for this step
			colliders.addAll(blobColliders);
		}
	}
