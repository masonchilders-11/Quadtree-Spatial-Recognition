 ...
 * @author Mason Childers, 23W, testing quadtree
 */
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// if the tree isn't initialized, initialize the tree with the first point
			if (tree == null) tree = new PointQuadtree<>(new Dot(x, y), 0, 0, width, height);
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0,800,600); // start with A
		tree.insert(new Dot(405,295)); // B
		tree.insert(new Dot(395,305)); // C
		tree.insert(new Dot(405,305)); // D
		tree.insert(new Dot(395,295)); // E
		tree.insert(new Dot(780,20)); // F
		tree.insert(new Dot(20,20)); // G
		tree.insert(new Dot(20,580)); // H
		tree.insert(new Dot(780,580)); // I
		int bad = 0;
		bad += testFind(400,300,10,9,9,5); 	// rect for all; circle for all; find A, B, C, D, E
		bad += testFind(400,300,900,9,9,9);	// rect for all; circle for all; find all
		bad += testFind(775,25,15,6,3,1);	// rect for A [B [F]] [C] [D] [E]; circle for A, B, F; find F
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		int treeX = (int)tree.getPoint().getX();
		int treeY = (int)tree.getPoint().getY();
		// drawing dot
		g.fillOval(treeX-dotRadius, treeY-dotRadius, 2*dotRadius, 2*dotRadius);
		// drawing lines
		g.drawLine(treeX, tree.getY1(), treeX, tree.getY2());
		g.drawLine(tree.getX1(), treeY, tree.getX2(), treeY);
